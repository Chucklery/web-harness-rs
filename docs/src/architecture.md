...
Ticket payloads and request digests live only in MCP-process memory. For the optional local-terminal fallback, a private bounded state directory stores only short-lived display summaries and one-shot approval markers; it does not contain the authority-bearing digest. The MCP transport can ask the Host for elicitation confirmation, but the model cannot call an approval tool. Without Host elicitation, the user must operate the fallback from a separate local terminal outside MCP `exec`. Expiry invalidates a ticket; denial removes it explicitly. A mismatching request — wrong capability, wrong argv, wrong cwd, or wrong background mode — fails without consuming the ticket, because a rejected request has not used the approval it was issued for. A retry with the correct payload therefore still succeeds within the five-minute window.
...
### Linux Landlock evaluation (not implemented)

Landlock is a plausible no-daemon Linux filesystem restriction and requires no elevated privilege, but it is not a drop-in Seatbelt equivalent. Its ABI must be queried at runtime because filesystem rights and TCP/UDP network controls were added in different ABI versions, and its documented limitations include metadata-changing operations such as `chmod`. A filesystem-only Landlock layer could be evaluated as defense in depth while retaining per-exec user approval; it must not cause Linux commands to be described as fully sandboxed or remove that approval until the exact guarantee is validated on supported Linux hosts. The initial implementation would use the existing `libc` dependency and direct syscalls, not a new runtime dependency. See the [upstream Landlock userspace documentation](https://docs.kernel.org/userspace-api/landlock.html).
