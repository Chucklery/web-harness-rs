...
Clients without elicitation support receive `approval_required` and must wait for a host-side approval mechanism; web-harness does not silently execute the operation. When that Host has no confirmation flow, use a separate local terminal: `web-harness approvals` lists pending requests, then `web-harness approve <ticket-id>` displays the capability and summary and requires an interactive `y` confirmation. The ticket and its request digest remain in MCP-process memory; a bounded, private state directory carries only the short-lived summary and one-shot CLI approval marker. The CLI refuses non-interactive stdin/stdout and never appears as an MCP tool. Tickets expire after five minutes and are consumed after one successful authorization.
