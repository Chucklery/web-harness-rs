...
Exec checks existing workspace path arguments against the protected-path policy before spawn. The check resolves symlinks before classifying the target, so a workspace-local alias to a protected file also requires explicit approval. This is a targeted safeguard for direct path arguments, not a general-purpose parser for arbitrary program behavior; script mode always requires explicit approval.
