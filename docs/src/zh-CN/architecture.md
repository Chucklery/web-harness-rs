...
`RuntimeTool` 接口与 `RuntimeRegistry` 现在统一调度全部 10 个直接工具：Workspace 信息/指令、文件读取、有界文件枚举、Search、Patch、Exec、Job 控制、Git 与审批票据管理。直接 MCP 调用和 adaptive-runtime 兼容网关共用同一 Registry 调用路径。
