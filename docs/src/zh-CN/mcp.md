...
结构化 Git 支持读取 revision 中的单个工作区文件、创建并切换分支，以及携带 `expected_head` 的 mutation；审批完成并在执行前会再次核验 HEAD，分支在审批期间变化时返回 Conflict。Commit 可携带 pathspec，避免把范围之外已有的暂存内容一并提交。
